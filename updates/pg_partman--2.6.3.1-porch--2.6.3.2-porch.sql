...
INSERT INTO partman_preserve_privs_temp
SELECT 'GRANT EXECUTE ON FUNCTION @extschema@.create_sub_parent(text, text, text, text, text[], integer, text, boolean, text, text, boolean, boolean, boolean, boolean)  TO '||array_to_string(array_agg(grantee::text), ',')||';'
FROM information_schema.routine_privileges
WHERE routine_schema = '@extschema@'
...
DROP FUNCTION @extschema@.create_sub_parent(text, text, text, text, text[], integer, text, boolean, text, text, boolean, boolean, boolean);
...
ALTER TABLE @extschema@.part_config_sub
    ADD COLUMN sub_audit_log BOOLEAN NOT NULL DEFAULT FALSE;
...
-- Do not create the child table if it's outside the bounds of the top parent.
    ...
    -- Porch Audit Logging
    PERFORM db_build.create_table(v_parent_schema, v_partition_name, v_audit_log);
    IF v_audit_log THEN
